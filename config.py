...
class MyOutOfOrderCore(BaseCPUCore):
    def __init__(self, width, rob_size, num_int_regs, num_fp_regs):
        ...
        self.core.branchPred = BranchPredictor(
            conditionalBranchPred=TournamentBP(numThreads=self.core.numThreads)
        )
...
# 定义一个新的Out-of-Order CPU处理器组件(只创建一个核心)
